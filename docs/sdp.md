...
| M1d | HMI presentation package (SCR-006, frontend-only), dispositions of the 2026-07-19 SPR campaign: causal log ordering (SPR-001 fix, sorted insertion), dedicated failure-code column (SPR-002), numeric dropdown ordering (SPR-004), widened layout for single-line hex (SPR-005), log heading alignment (SPR-006); gate additionally records the SIM-TC-034 re-run closing SPR-003 | SIM-TC-037/038 pass; amended SIM-TC-032/036 re-verified; SIM-TC-034 re-run recorded (SPR-003 closure); existing automated suite green; SPR-001…006 closed in the register; SRS M1d-scope requirements all traced+passed |
| M1e | Repository link and mobile usability package (SCR-007, frontend-only): permanent link from the console to the public source repository; layout reflow down to narrow mobile viewports (order of 360 px CSS width) — stacked header/compose/log controls, no page-level horizontal scrolling, wide log content confined to its container; desktop presentation unchanged | SIM-TC-039/040 pass; SIM-TC-038 unaffected (desktop presentation unchanged); existing automated suite green; SRS M1e-scope requirements all traced+passed |
